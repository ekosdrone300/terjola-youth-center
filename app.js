...
// 💥 Toast ფუნქცია (alert-ების ჩასანაცვლებლად)
  ...
  try {
      ...
      if (action === "requestDeleteStudent") {
          await db.collection("students").doc(data.docId).update({ 
              deleteRequest: true, 
              deleteRequestedBy: data.teacher, 
              deleteRequestedClub: data.club 
          });
      ...
      if (action === "rejectDelete") {
          await db.collection("students").doc(data.docId).update({ 
              deleteRequest: firebase.firestore.FieldValue.delete(), 
              deleteRequestedBy: firebase.firestore.FieldValue.delete(), 
              deleteRequestedClub: firebase.firestore.FieldValue.delete() 
          });
          clearCache('students'); return "success";
      }

      if (action === "deleteStudentByDoc") {
          await db.collection("students").doc(data.docId).delete(); 
          clearCache('students'); return "success";
      }

      if (action === "markOldStudent") {
          await db.collection("students").doc(data.docId).update({ isOldStudent: data.isOld });
          clearCache('students');
          return "success";
      }
...
window.toggleOldStudent = async function(docId, cb) {
    let isOld = cb.checked;
    cb.disabled = true;
    try {
        await apiCall("markOldStudent", { docId: docId, isOld: isOld });
        if(appCache.teacherStudents) {
            let st = appCache.teacherStudents.find(x => x.docId === docId);
            if(st) st.isOldStudent = isOld;
        }
        if(appCache.unifiedList) {
            let st = appCache.unifiedList.find(x => x.docId === docId);
            if(st) st.isOldStudent = isOld;
        }
        if(appCache.dpoList) {
            let st = appCache.dpoList.find(x => x.docId === docId);
            if(st) st.isOldStudent = isOld;
        }
        loadStudents(currentClub, false); 
...
window.requestDelete = async function(docId) {
    ...
    if(msg.includes("success")) showToast("წაშლის მოთხოვნა გაიგზავნა ადმინისტრატორთან!", "success");
...
window.approveDelete = async function(docId) {
    ...
    showToast("მოსწავლე წარმატებით წაიშალა სისტემიდან.", "success");
    loadUnifiedList(true);
    loadAdminStats(true);
}
...
window.rejectDelete = async function(docId) {
    ...
    showToast("წაშლის მოთხოვნა გაუქმებულია.", "info");
    loadUnifiedList(true);
}
...
async function submitRegistration() {
   ...
   let fullParentName = pName + " " + pSurname;
   
   const payloadData = { 
       name: document.getElementById('sName').value, 
       surname: document.getElementById('sSurname').value, 
       personalId: document.getElementById('sPId').value, 
       birthDate: document.getElementById('sBirthDate').value, 
       gender: document.getElementById('sGender').value, 
       school: document.getElementById('sSchool').value, 
       classNum: document.getElementById('sClass').value, 
       gradeLevel: document.getElementById('sGradeLevel').value, 
   ...
   if(msg.includes("success")) {
       showToast("წარმატებით დარეგისტრირდით!", "success");
       goHome();
   } else {
       showToast(msg, "error");
   }
...
async function loginSysAdmin() {
   ...
   if(isValid.success) { 
       document.getElementById('sysLoginForm').style.display = 'none'; 
       document.getElementById('sysContent').style.display = 'block'; 
       switchSysTab('Clubs'); 
       showToast("ავტორიზაცია წარმატებულია!", "success");
   } else showToast("პაროლი არასწორია!", "error");
}
...
async function addSysClub() { 
    await apiCall("addSysClub", { name: document.getElementById('newClubName').value, limit: document.getElementById('newClubLimit').value, schedule: document.getElementById('newClubSchedule').value }); 
    showToast("კლუბი წარმატებით დაემატა!", "success"); 
    loadSysClubs(true); 
}
...
async function addSysStaff() { 
    ...
    await apiCall("addSysStaff", { name: name, email: email, pass: pass, club: club }); 
    showToast("მასწავლებელი წარმატებით დაემატა!", "success"); 
    loadSysStaff(true); 
}
...
async function loginAdmin() { 
    if(await apiCall("checkAdminPass", { pass: document.getElementById('aPass').value })) { 
...
async function loadAdminStats(force = false) { 
    ...
    <div style="display:flex; gap:15px; margin-bottom:20px; flex-wrap:wrap;">
        <div style="flex:1; min-width:150px; background:linear-gradient(135deg, var(--primary), #2563eb); color:white; padding:20px; border-radius:14px; box-shadow: 0 10px 15px rgba(59,130,246,0.2); position:relative; overflow:hidden;">
            ...
            <h3 style="margin:0; font-size:1.1rem; opacity:0.9;">სულ მოსწავლე</h3>
            <h2 style="margin:10px 0 0 0; font-size:2.2rem;">${s.total}</h2>
        </div>
        <div style="flex:1; min-width:150px; background:linear-gradient(135deg, var(--secondary), #059669); color:white; padding:20px; border-radius:14px; box-shadow: 0 10px 15px rgba(16,185,129,0.2); position:relative; overflow:hidden;">
            ...
            <h3 style="margin:0; font-size:1.1rem; opacity:0.9;">ახალი (მიმდინარე)</h3>
            <h2 style="margin:10px 0 0 0; font-size:2.2rem;">${s.newStudents}</h2>
        </div>
        <div style="flex:1; min-width:150px; background:linear-gradient(135deg, #f59e0b, #d97706); color:white; padding:20px; border-radius:14px; box-shadow: 0 10px 15px rgba(245,158,11,0.2); position:relative; overflow:hidden;">
            ...
            <h3 style="margin:0; font-size:1.1rem; opacity:0.9;">ადრეული (ძველი)</h3>
            <h2 style="margin:10px 0 0 0; font-size:2.2rem;">${s.oldStudents}</h2>
        </div>
    </div>
...
function downloadAdminCSV() {
    ...
    showToast("ფაილი იტვირთება...", "success");
}

async function loginDpo() { 
    if(await apiCall("checkDpoPass", { pass: document.getElementById('dPass').value })) { 
        document.getElementById('dpoLoginForm').style.display = 'none'; 
        document.getElementById('dpoContent').style.display = 'block'; 
        loadDpoData(); 
        showToast("ავტორიზაცია წარმატებულია!", "success");
    } else showToast("პაროლი არასწორია!", "error"); 
}
async function loginTeacher() { 
    ...
    if(r.status === 'success'){ 
        currentClub = r.club; currentTeacher = r.teacherName; 
        showSection('teacherDashboard'); document.getElementById('dashTitle').innerText=currentClub; 
        loadStudents(r.club); 
        showToast("ავტორიზაცია წარმატებულია!", "success");
    } else showToast("არასწორი მონაცემები!", "error"); 
}
...
async function loadStudents(c, force = false) { 
   try {
       ...
       const rows = l.map(s => {
           ...
           let deleteBtn = s.deleteRequest ? 
               `<button class="del-btn" style="background:#94a3b8; cursor:not-allowed;" disabled>⏳ მოთხოვნილია</button>` : 
               `<button class="del-btn" onclick="requestDelete('${s.docId}')"><i class="fas fa-trash"></i> წაშლა</button>`;

           let stuData = encodeURIComponent(JSON.stringify({ date: s.regDate || "", name: `${s.name} ${s.surname}`, pId: s.pId, school: s.school || "", classNum: s.classNum || "", parentName: s.parentName || "", phone: s.phone || "", clubs: (s.clubsStr || (s.clubs ? s.clubs.join(", ") : "")), club: currentClub, uId: s.uId || "" }));
           ...
           return `<tr>
              ...
              <td class="print-hide">
                 <label style="font-size:11px; display:flex; align-items:center; gap:5px; cursor:pointer; color:#475569; background:#f1f5f9; padding:4px 8px; border-radius:6px; width:max-content;">
                    <input type="checkbox" onchange="toggleOldStudent('${s.docId}', this)" ${s.isOldStudent ? 'checked' : ''} style="width:14px; height:14px; margin:0;">
                    ადრეული
                 </label>
              </td>
              ...
              <td class="print-hide" style="text-align:center; white-space:nowrap;">
                  ...
                  <button class="doc-btn" title="თანხმობის გადმოწერა" onclick="generateDpoDoc('${stuData}')" style="background:#f59e0b;"><i class="fas fa-file-download"></i> DPO</button>
              </td>
           </tr>`;
       }).join('');
...
async function saveAttendance() {
   ...
   showToast("დასწრება წარმატებით შეინახა!", "success");
...
async function loadUnifiedList(force = false) { 
    ...
    const rows = l.map(s => {
        ...
        let stuData = encodeURIComponent(JSON.stringify({ date: s.regDate || "", name: `${s.name} ${s.surname}`, pId: s.pId, school: s.school || "", classNum: s.classNum || "", parentName: s.parentName || "", phone: s.phone || "", clubs: s.clubsStr, club: s.clubsStr, uId: s.uId || "" }));
...
async function loadDpoData(force = false) { 
    ...
    const rows = l.map(s => {
        ...
        let stuData = encodeURIComponent(JSON.stringify({ date: s.regDate || "", name: `${s.name} ${s.surname}`, pId: s.pId, school: s.school || "", classNum: s.classNum || "", parentName: s.parentName || "", phone: s.phone || "", clubs: clubsJoined, club: clubsJoined, uId: s.uId || "" }));
...
function generateDpoDoc(encodedDataStr) {
    ...
    <style> @media print { @page { size: A4; margin: 12mm; } } body { font-family: 'Sylfaen', serif; padding: 0; max-width: 800px; margin: auto; line-height: 1.35; color: #000; font-size: 13px;} p { margin-bottom: 10px; text-align: left; margin-top: 0; } .reg-id { text-align: right; font-size: 12px; font-family: sans-serif; color: #000; font-weight: bold; } .header-text { text-align: center; font-size: 13px; margin-bottom: 20px; font-weight: bold; } .title { text-align: left; font-weight: bold; margin-bottom: 15px; font-size: 14px;} .blank-line { display: inline-block; min-width: 300px; border-bottom: 1px solid #000; text-align: center; padding: 0 10px; font-weight: bold; } .small-label { display: block; font-size: 11px; margin-left: 145px; color: #333; margin-bottom: 15px;} .child-data-box { border-top: 1px dashed #000; border-bottom: 1px dashed #000; margin: 15px 0; padding: 10px 0; text-align: center; font-weight: bold; font-size: 14px; } .list-section { text-align: left; margin-bottom: 15px; } .list-section ul { margin: 5px 0; padding-left: 20px; } .list-section li { margin-bottom: 4px; } .sig-line { display: inline-block; font-weight: bold; font-size: 15px; color: #000; } .date-blank { text-decoration: none; border-bottom: 1px solid #000; padding: 0 8px; } </style>
      ...
      <div style="display:flex; align-items:flex-end; gap:5px;">
          <b>მე, ქვევით ხელმომწერი</b> <div style="border-bottom: 1px solid #000; width: 300px; text-align:center;"></div>
      </div>
      ...
      <div style="display: flex; align-items: flex-end; gap: 15px;">
          <div>მშობლის/კანონიერი წარმომადგენლის ხელმოწერა:</div>
          <div style="border-bottom: 1px solid #000; width: 200px;"></div>
          <div style="font-weight:bold;">/ ${s.parentName} /</div>
      </div>
...
function generateAppDoc(encodedDataStr) {
    ...
    <style> @media print { @page { size: A4; margin: 20mm; } } body { font-family: 'Sylfaen', serif; padding: 0; max-width: 800px; margin: auto; line-height: 1.6; color: #000; font-size: 15px;} .doc-wrapper { padding: 10px; } .reg-id { text-align: right; font-size: 13px; font-family: sans-serif; color: #000; font-weight: bold; border-bottom: 2px solid #000; padding-bottom: 5px; } .clearfix::after { content: ""; clear: both; display: table; } .header-right { text-align: right; margin-left: auto; width: 60%; font-size: 14px; margin-bottom: 50px; line-height: 1.5; margin-top: 20px;} .title { text-align: center; font-weight: bold; font-size: 22px; letter-spacing: 8px; margin: 40px 0 50px 0;} .body-text { text-align: justify; line-height: 2.2; font-size: 15px; } .blank-line { display: inline-block; border-bottom: 1px dashed #000; text-align: center; padding: 0 10px; font-weight: bold; color: #000; text-decoration: none; min-width: 50px;} .footer-right { text-align: left; margin-left: 60%; margin-top: 70px; line-height: 2.2; font-size: 15px;} .sig-line { display: inline-block; font-weight: bold; font-size: 15px; color: #000; } </style>
      ...
      <div class="header-right">ა (ა) იპ თერჯოლის მოსწავლე-ახალგაზრდობის<br>სოციალური დაცვის, მოქალაქეობრივი და ესთეტიკური<br>აღზრდის მუნიციპალური ცენტრის დირექტორს<br>ქალბატონ ნათია მოსიაშვილს<br>
      <div style="font-size: 12px; margin-top: 5px;">(მშობლის სახელი, გვარი)</div>
      <div class="blank-line" style="min-width: 250px; text-align: right; margin-top: 5px;"></div></div>
      ...
      <div class="footer-right">
          თარიღი: <span class="blank-line" style="min-width: 150px;">${day} / ${month} / 20${year} წ.</span><br>
          <div style="display: flex; align-items: flex-end; gap: 10px; margin-top: 5px;">
              <div>ხელმოწერა:</div>
              <div style="border-bottom: 1px dashed #000; width: 150px;"></div>
              <div style="font-weight:bold;">/ ${s.parentName} /</div>
          </div>
          საკონტაქტო: <span class="blank-line" style="min-width: 150px; margin-top: 5px;">${s.phone}</span>
      </div>
     </div><script>setTimeout(() => { window.print(); }, 800);<\/script></body></html>`;
    docWindow.document.write(htmlContent); docWindow.document.close();
}