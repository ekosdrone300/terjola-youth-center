...
// 💥 შეტყობინებების გამოწერა (დაცული ვერსია)
  ...
  try {
      ...
      if (action === "registerStudent") {
          ...
          if (!p.isSpecial) {
              ...
              // 💥 პუშის გაგზავნა Vercel-ის API-თ
              try {
...
window.requestDeleteFromDpo = async function(docId) {
    if(!confirm("ნამდვილად გსურთ ადმინისტრატორთან წაშლის მოთხოვნის გაგზავნა?")) return;
    let msg = await apiCall("requestDeleteStudent", { docId: docId, teacher: "DPO ოფიცერი", club: "-" });
    if(msg.includes("success")) showToast("წაშლის მოთხოვნა გაიგზავნა!", "success");
    loadDpoData(true);
}
...
async function loadDpoData(force = false) { 
    ...
    const header = `<div style="display:flex; justify-content:space-between; align-items:center; margin-bottom:10px;">
             <div style="font-weight:bold; color:var(--primary); font-size:15px; background:#f1f5f9; padding:8px 12px; border-radius:8px;">
                 <i class="fas fa-users"></i> სულ რეგისტრირებული: ${l.length}
             </div>
             <button class="admin-btn print-hide" onclick="loadDpoData(true)" style="width:max-content;"><i class="fas fa-sync"></i> განახლება</button>
         </div>
...
// 💥 PWA Service Worker (უსაფრთხო ავტო-განახლებით და ვერსიის ამოკითხვით)
if ('serviceWorker' in navigator) {
  window.addEventListener('load', () => {
    navigator.serviceWorker.register('/sw.js')
      .then(registration => {
        console.log('SW დარეგისტრირდა', registration.scope);
        ...
        registration.addEventListener('updatefound', () => {
          ...
          newWorker.addEventListener('statechange', () => {
            if (newWorker.state === 'installed' && navigator.serviceWorker.controller) {
               ...
               // კითხულობს ვერსიას index.html-ის ფუტერიდან
               let versionText = "ახალ ვერსიაზე";
               const footer = document.querySelector('.app-footer');
               if (footer) {
                   let match = footer.innerText.match(/\d+\.\d+\.\d+/);
                   if (match) versionText = match[0] + " ვერსიაზე";
               }
