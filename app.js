...
window.onload = async function() {
  ...
  // სისტემის სტატუსის რეალურ დროში მოსმენა
...
// 💥 სისტემური ლოგიკა და რეჟიმის შეცვლა
...
window.toggleSysStatus = async function(field, val) {
    try {
        await db.collection("system").doc("status").set({ [field]: val }, { merge: true });
        showToast("ტექნიკური რეჟიმი შეიცვალა", "success");
    } catch(e) { showToast("შეცდომა რეჟიმის შეცვლისას", "error"); }
}
...
async function loginTeacher() { 
    ...
    if(r.status === 'success'){ currentClub = r.club; currentTeacher = r.teacherName; showSection('teacherDashboard'); document.getElementById('dashTitle').innerText=currentClub; loadStudents(r.club); showToast("ავტორიზაცია წარმატებულია!", "success"); } else showToast("არასწორი მონაცემები!", "error"); 
}
