...
async function apiCall(action, data = {}) {
  try {
      ...
      // 💥 მასწავლებლის მიერ მოსწავლის წაშლის მოთხოვნის გაგზავნა
      ...
      // 💥 ადმინისტრატორის მიერ მოთხოვნის უარყოფა (გაუქმება)
      ...
      // 💥 ადმინისტრატორის მიერ საბოლოოდ წაშლა Document ID-თ
...
// 💥 წაშლის მოთხოვნის და დადასტურების ფუნქციები
window.requestDelete = async function(docId) {
    ...
    alert(msg);
    loadStudents(currentClub, true); // განახლება სერვერიდან
}
...
window.approveDelete = async function(docId) {
    ...
    alert("მოსწავლე წარმატებით წაიშალა სისტემიდან.");
...
window.rejectDelete = async function(docId) {
    ...
    alert("წაშლის მოთხოვნა გაუქმებულია.");
    loadUnifiedList(true);
}
...
async function loadSysStudents(force = false) { 
    ...
    const rows = l.map(s => {
        ...
        return `<tr><td>${s.uId}</td><td>${s.surname} ${s.name} ${oldBadge}</td><td><button class="del-btn" onclick="apiCall('deleteStudent', {uId:'${s.uId}'}).then(()=>loadSysStudents(true))">ბაზიდან წაშლა</button></td></tr>`;
    }).join('');
...
async function loadStudents(c, force = false) { 
   try {
       ...
       const rows = l.map(s => {
           ...
           // 💥 წაშლის ღილაკის ლოგიკა მასწავლებელთან
...
async function loadUnifiedList(force = false) { 
    ...
    const rows = l.map(s => {
        ...
        // 💥 წაშლის მოთხოვნის ვიზუალი ადმინთან
