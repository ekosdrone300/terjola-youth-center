...
async function apiCall(action, data = {}) {
  try {
      ...
      // ძველი მოსწავლის სტატუსის განახლება
      ...
      if (action === "getStudentsForTeacher") {
          ...
          let list = []; 
          snap.forEach(d => { let st = d.data(); list.push({ ...st, docId: d.id, isOldStudent: st.isOldStudent || false, otherClubs: st.clubs.filter(c => c !== data.clubName).join(", ") || "არა", timestamp: st.timestamp }); }); 
      ...
      if (action === "getUnifiedData" || action === "getDpoData") {
          const snap = await db.collection("students").get();
          let list = []; 
          snap.forEach(d => { let st = d.data(); list.push({ ...st, isOldStudent: st.isOldStudent || false, clubsStr: st.clubs.join(", "), docId: d.id, timestamp: st.timestamp }); }); 
          list.sort((a, b) => (b.timestamp?.toMillis() || 0) - (a.timestamp?.toMillis() || 0)); return list;
      }
...
async function submitRegistration() {
   // ვალიდაციას დაემატა sParentSurname
   ...
   // სახელის და გვარის შეერთება ბაზაში გასაგზავნად
   ...
   const payloadData = { 
       ...
       parentName: fullParentName, // ინახება შეერთებული
...
async function loadAdminStats(force = false) { 
    ...
    // 💥 სტატისტიკის ახალი დიზაინი (ცალ-ცალკე ბარათებით)
    let h = `
...
async function loadStudents(c, force = false) { 
   try {
       ...
       const header = `<div style="display:flex; justify-content:space-between; margin-bottom:15px;">
                ...
                <table class="data-table"><tr><th><i class="fas fa-check"></i></th><th>მოსწავლე</th><th>პირადი N</th><th class="print-hide">სტატუსი</th><th class="print-hide" style="text-align:center;">დოკუმენტები</th></tr>`; 
                
       const rows = l.map(s => {
           let badge = getNewBadge(s.timestamp);
           let oldBadge = s.isOldStudent ? `<span class="old-badge"><i class="fas fa-history"></i> ძველი</span>` : '';
           
           // 💥 მონაცემების შიფრაცია (quotes error fix)
...
async function loadUnifiedList(force = false) { 
    ...
    const rows = l.map(s => {
        ...
        return `<tr><td>${s.uId}</td><td><b>${s.surname} ${s.name}</b> ${badge} ${oldBadge}</td><td>${s.pId}</td><td>${s.clubsStr}</td>
              <td class="print-hide" style="text-align:center; white-space:nowrap;">
                  <button class="doc-btn" title="განცხადების გადმოწერა" onclick="generateAppDoc('${stuData}')" style="background:var(--primary); margin-right:5px;"><i class="fas fa-file-download"></i> განცხ.</button>
                  <button class="doc-btn" title="თანხმობის გადმოწერა" onclick="generateDpoDoc('${stuData}')" style="background:#f59e0b;"><i class="fas fa-file-download"></i> DPO</button>
              </td></tr>`;
