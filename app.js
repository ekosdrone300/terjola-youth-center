...
async function loadAdminStats(force = false) { 
    ...
    <div style="background:linear-gradient(135deg, var(--secondary), #059669); color:white; padding:20px; border-radius:14px; margin-bottom:20px; display:flex; justify-content:space-between; align-items:center; box-shadow: 0 10px 15px rgba(16,185,129,0.2); flex-wrap:wrap; gap:10px;">
        <div>
            ...
            <div style="margin-top:8px; font-size:14px; background:rgba(0,0,0,0.15); display:inline-block; padding:4px 10px; border-radius:8px;">ახალი: <b>${s.newStudents}</b> &nbsp;|&nbsp; ადრეული (ძველი): <b>${s.oldStudents}</b></div>
        </div>
        <button onclick="loadAdminStats(true)" class="print-hide" style="background:rgba(255,255,255,0.2); border:none; color:white; padding:8px 15px; border-radius:8px; cursor:pointer; font-weight:bold;"><i class="fas fa-sync"></i> განახლება</button>
    </div>
...
async function loadStudents(c, force = false) { 
   try {
       ...
       const rows = l.map(s => {
           ...
           // სრული ობიექტი ორივე დოკუმენტისთვის
           let stuData = JSON.stringify({ date: s.regDate || "", name: `${s.name} ${s.surname}`, pId: s.pId, school: s.school || "", classNum: s.classNum || "", parentName: s.parentName || "", phone: s.phone || "", clubs: (s.clubsStr || (s.clubs ? s.clubs.join(", ") : "")), club: currentClub, uId: s.uId || "" }).replace(/"/g, '&quot;');
...
async function loadUnifiedList(force = false) { 
    ...
    const rows = l.map(s => {
        ...
        let stuData = JSON.stringify({ date: s.regDate || "", name: `${s.name} ${s.surname}`, pId: s.pId, school: s.school || "", classNum: s.classNum || "", parentName: s.parentName || "", phone: s.phone || "", clubs: s.clubsStr, club: s.clubsStr, uId: s.uId || "" }).replace(/"/g, '&quot;');
...
async function loadDpoData(force = false) { 
    ...
    const rows = l.map(s => {
        ...
        let stuData = JSON.stringify({ date: s.regDate || "", name: `${s.name} ${s.surname}`, pId: s.pId, school: s.school || "", classNum: s.classNum || "", parentName: s.parentName || "", phone: s.phone || "", clubs: clubsJoined, club: clubsJoined, uId: s.uId || "" }).replace(/"/g, '&quot;');
