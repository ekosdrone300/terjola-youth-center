...
// 💥 PWA Service Worker (უსაფრთხო ავტო-განახლებით)
  ...
  window.addEventListener('load', () => {
    navigator.serviceWorker.register('/sw.js')
      .then(registration => {
        ...
        // როდესაც ახალ ვერსიას იპოვის სერვერზე
        registration.addEventListener('updatefound', () => {
          const newWorker = registration.installing;
          newWorker.addEventListener('statechange', () => {
            // თუ ახალი ვერსია ჩამოიტვირთა და მზადაა
            if (newWorker.state === 'installed' && navigator.serviceWorker.controller) {
               showToast("სისტემა ახლდება ახალ ვერსიაზე...", "info");
               // ვეუბნებით Service Worker-ს, რომ გააქტიურდეს
      ...
      // 💥 დამცავი მექანიზმი (Lock), რომ Safari არ გაიჭედოს ციკლში
      let refreshing = false;
      navigator.serviceWorker.addEventListener('controllerchange', () => {
        if (!refreshing) {
          refreshing = true;
          window.location.reload(); // არესტარტებს საიტს მხოლოდ ერთხელ!
        }
      });
  });
}