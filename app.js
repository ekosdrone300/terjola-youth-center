...
// FCM შეტყობინებების გამოწერა კლუბების ფილტრაციით
async function subscribeToNotifications() {
    try {
        const permission = await Notification.requestPermission();
        if (permission === 'granted') {
            const currentToken = await messaging.getToken({ vapidKey: 'BJyLCIV9109r2E6Q7eHJmipJr262hfltBZDX8t16vcyd5j9zuy9Bw6VXuc_8pUE_NOMxOK8pPtJzEfGQFNSmPTM' });
            if (currentToken) {
                // ვინახავთ ტოკენს და კლუბს Vercel-ის ფილტრაციისთვის
...
if ('serviceWorker' in navigator) {
  window.addEventListener('load', () => {
    navigator.serviceWorker.register('/sw.js')
      .then(registration => {
        console.log('PWA ServiceWorker წარმატებით დარეგისტრირდა', registration.scope);
      })
      .catch(error => {
        console.log('PWA ServiceWorker-ის რეგისტრაცია ვერ მოხერხდა:', error);
      });
  });
}